...
public abstract class BaseDao extends SqlSessionDaoSupport {
     ...
     * @return
     */
    public List selectList(String statement, Object parameter, int nub, int size) {
        SqlSession session = openSession();
        return session.selectList(statement, parameter,
                new RowBounds(nub, size));
    }
    ...
    public int insert(String arg0, Object arg1) {
        ...
        if (row > 1) {
            ...
            // if (arg1 instanceof Base) {
            // return Integer.valueOf(((Base) arg1).getId());
            // } else {
            return row;
        }else if(row==0){
            return row;
